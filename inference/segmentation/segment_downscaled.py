...
device = "cuda"
data_path = Path("/home/fatbardhf/bicubical_A_20210707")	# Change the folder path
save_path_suffixes = ["2x", "3x", "4x"]  # Add suffixes for saving results
...
def combine_labelmap_from_slices(labelmap, grid=(22, 15), device="cuda"):
    ...
    if len(labelmap.shape) == 3:
        full_ann = torch.zeros(
            (grid[1] * labelmap.shape[-1], grid[0] * labelmap.shape[-1]),
            dtype=torch.int64,
            device=device
        )
        ...
        for i in range(grid[1]):
            for j in range(grid[0]):
                full_ann[
                    offset[1] * i: min(offset[1] * i + tile_size[1], full_ann.shape[0]),
                    offset[0] * j: min(offset[0] * j + tile_size[0], full_ann.shape[1])
                ] = labelmap[placement]
                placement += 1
    elif len(labelmap.shape) == 4:
        labelmap = labelmap.permute(0, 2, 3, 1)
        full_ann = torch.zeros(
            (grid[1] * labelmap.shape[-1], grid[0] * labelmap.shape[-1], 3),
            dtype=torch.int64,
            device=device
        )
        offset = (labelmap.shape[-1], labelmap.shape[-1])
...
def main():
    ...
    for suffix in save_path_suffixes:
        save_path = Path("inference/segmentation/results") / f"results_{suffix}"  # Change the folder path
        save_path.mkdir(parents=True, exist_ok=True)

        img_ls = sorted(list((data_path / suffix).rglob("*")))  # Update the image path
        ...
        for img_l in img_ls:
            ...
            for pred, fname in zip(preds, [img_l]):
                skio.imsave(
                    f"{str(save_path)}/{model_suffix}_{fname.split('/')[-1].split('.')[0]}_pred.png",
