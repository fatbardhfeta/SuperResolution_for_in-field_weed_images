...
def scale_images(original_images_dir, scaling_factors, new_images_dir = "inference/segmentation/results"):
    # Create folders for downsampled images
    new_images_dirs = []
    for factor in scaling_factors:
        new_images_factor_dir = os.path.join(new_images_dir, factor, "images")
    ...
    for filename in tqdm(original_images, desc="Downsampling images: ", unit="image"):
        ...
        for factor, new_images_factor_dir in zip(scaling_factors, new_images_dirs):
            ...
            downscaled_image.save(os.path.join(new_images_factor_dir, f"{filename}"))

    return new_images_dirs

def downscale_and_upscale_bicubicaly(original_images_dir, scaling_factors, new_images_dir = "inference/segmentation/results"):
    ...
    # Iterate over original images with progress bar
    for filename in tqdm(original_images, desc="Downsampling and upscaling images", unit="image"):
        # Load the original image
        original_image_path = os.path.join(original_images_dir, filename)
        original_image = Image.open(original_image_path)
        ...
        for factor, new_images_factor_dir in zip(scaling_factors, new_images_dirs):
            ...
            upscaled_image.save(os.path.join(new_images_factor_dir,f"{filename}"))
            #downscaled_image.save(os.path.join(new_images_factor_dir, f"{filename}"))
